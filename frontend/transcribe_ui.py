...
def upload_files_ui(column):
    ...
    with column:
        uploaded_files = st.file_uploader(
            "Upload audio/video file(s)", type=["wav", "mp3", "mp4", "m4a"], accept_multiple_files=True
        )
        if uploaded_files:
    ...
    with st.expander("Settings"):
        ...
        settings = {
            "size_of_model": st.selectbox(
                "Speech recognition model size (small is faster, large is more accurate)", ["small", "large"], index=1
            ),
            "task_str": st.selectbox("Task", ["transcribe", "translate"], index=0),
            "source_language": st.selectbox(
                "Source Language", options=languages, index=languages.index("english") if "english" in languages else 0
            ),
