...
openssl req -new -key $CLIENT_KEY -out $CLIENT_CSR -subj "/C=US/ST=State/L=City/O=Organization/CN=arbi-tr-api-container" -config <(cat /etc/ssl/openssl.cnf <(printf "[req]\ndistinguished_name=req_distinguished_name\nreq_extensions=req_ext\n[req_distinguished_name]\nCN=Common Name\n[req_ext]\nsubjectAltName=@alt_names\n[alt_names]\nDNS.1=arbi-tr-api-container\nDNS.2=localhost"))
